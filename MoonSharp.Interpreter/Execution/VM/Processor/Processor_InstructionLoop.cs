...
namespace MoonSharp.Interpreter.Execution.VM
{
	sealed partial class Processor
	{
		...
		{
            FixedNumber? v = m_ValueStack.Pop().ToScalar().CastToNumber();
			if (v.HasValue)
		...
		private int ExecAdd(Instruction i, int instructionPtr)
		{
            ...
            FixedNumber? rn = r.CastToNumber();
            FixedNumber? ln = l.CastToNumber();
		...
		private int ExecMod(Instruction i, int instructionPtr)
		{
			...
			if (ln.HasValue && rn.HasValue)
			{
                FixedNumber mod = FixedNumber.IEEERemainder(ln.Value, rn.Value);
		...
		private int ExecPower(Instruction i, int instructionPtr)
		{
			...
			if (ln.HasValue && rn.HasValue)
			{
				m_ValueStack.Push(DynValue.NewNumber(FixedNumber.Pow(ln.Value, rn.Value)));
				return instructionPtr;
			}
			else
			{
		...
		private int ExecIndexSet(Instruction i, int instructionPtr)
		{
			...
			while (nestedMetaOps > 0)
			{
				...
				else
				{
					...
					if (h == null || h.IsNil())
						throw ScriptRuntimeException.IndexType(obj, originalIdx.ToString());
				}
